...
_NPU_ATTENTION_MASK_2D_LAYER_NAMES = {'GatedDeltaNet'}
...
def needs_npu_attention_mask_2d(models: Iterable[Any]) -> bool:
    for model in models:
        for module in model.modules():
            if module.__class__.__name__ in _NPU_ATTENTION_MASK_2D_LAYER_NAMES:
                return True
    return False


def should_use_npu_generated_attention_mask(args) -> bool:
    attention_backend = getattr(args, 'attention_backend', None)
    attention_backend = getattr(attention_backend, 'name', attention_backend)
    return (is_torch_npu_available() and args.task_type == 'causal_lm' and not args.padding_free
            and attention_backend != 'local' and getattr(args, 'use_flash_attn', False))


def prepare_npu_attention_mask(batch: Dict[str, Any], *, needs_attention_mask_2d: bool) -> None:
    # NPU flash attention path: drop the 4D mask generated by the template in all cases.
    # The historical name `attention_mask_2d` is a bit misleading here: the value is a
    # [batch, seq] bool padding indicator, and only linear-attention layers such as
    # GatedDeltaNet consume it. Standard transformer blocks would receive it as an
    # unexpected kwarg, so remove it unless the built model has a known consumer.
    if needs_attention_mask_2d:
