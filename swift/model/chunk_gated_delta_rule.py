...
_RUNTIME_CU_SEQLENS: Optional[torch.Tensor] = None


@contextmanager
def use_runtime_cu_seqlens(cu_seqlens: Optional[torch.Tensor]):
    global _RUNTIME_CU_SEQLENS
    prev_cu_seqlens = _RUNTIME_CU_SEQLENS
    _RUNTIME_CU_SEQLENS = cu_seqlens
    try:
        yield
    finally:
        _RUNTIME_CU_SEQLENS = prev_cu_seqlens


def get_runtime_cu_seqlens() -> Optional[torch.Tensor]:
    return _RUNTIME_CU_SEQLENS
...
class ChunkGatedDeltaRuleFunction(torch.autograd.Function):
    ...
    def backward(
        ...
        if ctx.use_qk_l2norm_in_kernel:
            dq = dq.contiguous()
            dk = dk.contiguous()
            # Cast to float32 before l2norm_bwd to avoid bf16 overflow when
            # q_rstd is large (i.e., original q vector had small L2 norm).
            # In bf16, rstd * dy can exceed 65504 and produce inf → NaN.
            orig_dtype = dq.dtype
            dq = l2norm_bwd(q.float(), q_rstd.float(), dq.float()).to(orig_dtype)
            dk = l2norm_bwd(k.float(), k_rstd.float(), dk.float()).to(orig_dtype)
...
def chunk_gated_delta_rule(
    ...
    if cu_seqlens is None:
        cu_seqlens = get_runtime_cu_seqlens()
